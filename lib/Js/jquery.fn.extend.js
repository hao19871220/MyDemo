...
$.fn.extend({
    tooltip_btnlist: function (jsonArr) {
        ...
        // 自定义tooltip click事件
        //$(obj).click(function () {
        //    if ($(this).next('div').hasClass("tooltip")) {
        //        $(this).tooltip("hide");
        //    }
        //    else {
        //        $(this).parents('table:first').find('div[class*="tooltip"]').parent().find(obj).tooltip('hide');
        //        $(this).tooltip('show');
        //    }
        //});
    }
...
$.fn.extend({
    ...
    */
    getPageListWithSort: function (pageSize, webApi, query, callBack, sortColumn, isbigdata) {

        var lang = ReadCookie('lang');
        var sEmptyTable;
        var skip = '跳转';
        if (isbigdata == undefined || isbigdata == true) {
        ...
        //$(this).after('<div id="' + pagingid + '" style="text-align: right;float:left;width:100%;"></div>');
        ...
        if (query["LanguageKey"] == undefined || query["LanguageKey"] == null) {
            query["LanguageKey"] = lang;
        }
        ...
        var showpaging = function (maxcount) {
            ...
            if (maxcount <= 0) {
                //if ($("#amounttable").parents("table:first").find("tbody").length > 0) {
                //    pagingObj.parents("table:first").find("tbody").append("<tr><td>" + sEmptyTable + "</td></tr>");
                //}
                //else {
                //    pagingObj.parents("table:first").append("<tboby><tr><td>" + sEmptyTable + "</td></tr></tboby>");
                //}               
                var colnum = pagingObj.find("tr").eq(0).find("th").length;
                //var colnum = sortColumn.length;
